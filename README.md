...
> - **输出层和损失函数**：为像素级分类，即图像分割，输出层Softmax结构实现多分类，实际采用*log_softmax*，损失函数为负对数似然损失函数*NLLLoss*。公式如下: $log_softmax=\frac{e^{xi}}{\sum_{i}\ e^{xi}}NLLLoss=-\frac{1}{N}\sum_{k=1}^{N}{y_k\left(log_softmax\right)}$
